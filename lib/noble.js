...
Noble.prototype.onConnect = function(peripheralUuid, error) {
  ...
  if (error) {
    ...
    var hciStatus = error.hciStatusHex || 'unknown';
    this.emit(
      'warning',
      'connect failed peripheral=' + peripheralUuid + ' ' +
  ...
  if (peripheral) {
    ...
    if (!error) {
      this.emit(
        'warning',
        'unknown peripheral ' + peripheralUuid + ' connected! error=none hciStatus=unknown'
      );
    }
  }
};
