...
Noble.prototype.onConnect = function(peripheralUuid, error) {
  ...
  if (peripheral) {
    ...
    var errorMessage = error && error.message ? error.message : 'none';
    var hciStatus = error && error.hciStatusHex ? error.hciStatusHex : 'unknown';
    this.emit(
      'warning',
      'unknown peripheral ' + peripheralUuid + ' connected! ' +
      'error=' + errorMessage + ' hciStatus=' + hciStatus
    );
    if (error) {
      this.logAdapterFlagsForConnectFailure(error);
    }
  }
};

Noble.prototype.logAdapterFlagsForConnectFailure = function(error) {
  if (!error) {
    return;
  }
  var deviceId = typeof error.hciDeviceId === 'number' ? error.hciDeviceId : 0;
  var adapter = 'hci' + deviceId;
  childProcess.execFile('hciconfig', [adapter], function(commandError, stdout) {
    var details = commandError ?
      'unavailable error=' + commandError.message :
      stdout.trim().replace(/\s+/g, ' ');
    console.warn('noble: connect failure adapter=' + adapter + ' flags=' + details);
  });
};
